...
void floorplan(const char file[], const char outfile[]) {
  ...
  //node_t* root = challenge_init_slicing_tree();
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  else return is_in_subtree(a->right,b);
  return 0;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  //can just swap the module part, not the entire node
...
void swap_topology(node_t* a, node_t* b) {
  ...
  node_t * a_par = a->parent;
  node_t * b_par = b->parent;
  //1 means left child, 0 means right child
  int a_pos = (a_par->left==a) ? 1:0;
  int b_pos = (b_par->left==b) ? 1:0;

  a->parent = b_par;
  b->parent = a_par;
  if(a_pos == 1) a_par->left = b; else a_par->right = b;
  if(b_pos == 1) b_par->left = a; else b_par->right = a;
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  node_t* r_child = (node_t*)malloc(sizeof(node_t)); 
  r_child->module = &(modules[n]);
  r_child->cutline = UNDEFINED_CUTLINE;
  r_child->right = NULL;
  r_child->left = NULL;
  r_child->parent = ptr;
  ptr->right = r_child;
...
/*
 *  Helper fxn for challenge part. slightly modified ver. of original fxn.
 */
node_t* challengeHelper(node_t* par, int n) { 
  assert(n >= 0 && n < num_modules);
  node_t* ptr = (node_t*)malloc(sizeof(node_t));
  if(n == num_modules - 1) {
    ptr->module = &(modules[n]);
  ...
  ptr->module = NULL;
  //modifying something for challenge attempt..
  // if tall shape, make V. if wide shape, make H.
  ptr-> cutline = (modules[n].w >= modules[n].h) ? H : V;
  ptr->parent = par;
  node_t* r_child = (node_t*)malloc(sizeof(node_t)); 
  r_child->module = &(modules[n]);
  r_child->cutline = UNDEFINED_CUTLINE;
...
// helper fxn for challenge.
void bubbleSortModules(module_t* n){
    for(int i=num_modules-1;i>=0;--i){
        for(int j=1;j<=i;++j){
            if(n[j-1].w*n[j-1].h > n[j].w*n[j].h){
                int tempidx = n[j-1].idx;
                int tempx = n[j-1].llx;
                int tempy = n[j-1].lly;
                int tempw = n[j-1].w;
                int temph = n[j-1].h;
...
node_t* challenge_init_slicing_tree() {
  bubbleSortModules(modules);
  node_t* newRoot = challengeHelper(NULL,0);
  return newRoot;
}
